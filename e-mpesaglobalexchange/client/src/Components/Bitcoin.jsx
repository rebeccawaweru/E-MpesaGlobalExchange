...
export default function Bitcoin({sign ,id}){
    ...
    const getUser = async ()=>{
     ...
     useEffect(()=>{
      ...
      const handleChange = (e)=>{
       ...
       fetch(`https://blockchain.info/tobtc?currency=USD&value=1`, options)
       .then(response => response.json())
       .then(response=> {
         console.log(response)
         setReceive(e.target.value/response*98/100)
       })
      ...
      .then(response=> {
        console.log(response)
        setWithdraw(e.target.value/response*95.5/100)
      })
      .catch(error => console.log('error', error));
      }
      const handleChange3 = (e)=>{
        setAmount3(e.target.value)
      fetch(`https://blockchain.info/tobtc?currency=USD&value=${e.target.value}`, options)
      .then(response => response.json())
      .then(response=> {
        console.log(response)
        setWithdraw2(response*95.5/100)
      })
      .catch(error => console.log('error', error));
      }
    ...
    return(
        <>
         <Typography variant='h5' component="h4">Bitcoin</Typography>
       ...
       <CustomModal handleClose={()=>setOpen11(false)} title="CONFIRM"
        ...
        <CustomModal handleClose={()=>setOpen9(false)} open={open9}
         content={
          <>
          Deposit USD To Bitcoin Address: 
          <CustomTextField value="3KkS2Wbsh8FUNZ5swBmvBWXeBAw46Mk3Dc"/><Button sx={{mx:2,mt:1}} onClick={() => navigator.clipboard.writeText("3KkS2Wbsh8FUNZ5swBmvBWXeBAw46Mk3Dc")} variant="contained">Copy</Button>
          </>
         }/>
          <p>Amount in Bitcoin</p>
          <input type="number" value={amount} onChange={handleChange} placeholder="amount" style={style}/>
          <p>You will receive: ${receive.toFixed()} </p>
        ...
        <CustomModal handleClose={()=>setOpen8(false)} title="Withdraw Account"
         content={<>
          ...
          <input type="radio" value="Bitcoin" onChange={(e)=>setMethod(e.target.value)} />Bitcoin<br></br>
          ...
          <CustomTextField label="Bitcoin Address" value={accounttypenumber2} handleChange={(e)=>setAccounttypenumber2(e.target.value)}  />
          <br></br>
       ...
       {/* stop */}
          ...
          <p>You will receive BTC {" "}{withdraw2.toFixed(6)} </p>
          <hr style={{marginLeft:5,marginTop:30}}></hr>
          <Button variant="contained" sx={{mt:2}} onClick={handleWithdraw}>Withdraw</Button>
        </CustomContent>
        <CustomContent image={bitcoin} width="200" height={380}>
          <Typography component="h5" variant="h5" sx={{fontSize:30}}>Sell</Typography>
          <Typography component="h5" variant="h5" sx={{fontSize:30}}>Bitcoin</Typography>
          <hr style={{marginLeft:5}}></hr>
          <CustomModal handleClose={()=>setOpen3(false)} title="Binary Account"
         content={<>
          {error && <Alert color='error'>{error}</Alert>}
        ...
        <CustomModal handleClose={()=>setOpen8(false)} title="Withdraw Account"
         ...
         open={open8}/>
          ...
          <br></br><br></br>
          <p>You will receive:${" "}{withdraw.toFixed()}  </p>
